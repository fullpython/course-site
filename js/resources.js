  ...
  */
  ...
  var $tr = $('<tr>').append(
    ...
    $('<td>').append(
      ...
      create_anchor(lecture.animated, "animated (beta)", "N/A")
    )
...
function create_week_heading_tr(week) {
  ...
  */
  ...
  var friday = week.days[2];

  var start = new Date(week.dates.start);
  var end = new Date(week.dates.end);
  var startf = (start.getDate()) + '/' + (start.getMonth() + 1);
  var endf = (end.getDate()) + '/' + (end.getMonth() + 1);
  var $tr = $('<tr>').append(
    $('<td>').append(
  ...
  var $friday = $('<td>').append(
    create_day_markup(friday)
  );
  if (friday.active) {
    $friday.addClass('success');
  }

  return $tr.append($monday).append($wednesday).append($friday);
}
